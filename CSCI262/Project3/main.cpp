...
int main() {
    ...
    while (true) {      
        ...
        int length = get_length("How long of a word would you like?");
        bool showPossibleWords = get_yesno("Show running total of words? (y/n)");
        ...
        while (!game.is_won() && !game.is_lost()) {
            ...
            if(showPossibleWords){
                cout << game.print_remaining() << endl;
            }
            if (good_guess) {
